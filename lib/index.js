...
function pixelsJS() {
    ...
    return {
        ...
        filterImg: (img, filter, useNaturalDimensions = false) => {
            const canvas = doFilterImg(img, filter, useNaturalDimensions);
            if(canvas !== null) {
        ...
        // Browser-only function
        // Instead of replacing the img tag with a canvas, return a blob
        // representing the new image data to not be limited to replacing
        // the img tag
        filterImgAsBlob: (img, filter, useNaturalDimensions = false) => {
            const canvas = doFilterImg(img, filter, useNaturalDimensions);
            if(canvas !== null) {
...
// Browser-only function
const doFilterImg = (img, filter, useNaturalDimensions) => {
    let filterExists = checkIfFilterExists(filter);
    if (filterExists) {
        let filter_dict = getFilterNames();
